...
function typeEmoji(type) {
  return { Wedding: "💍", "Club Night": "🎷", "Private Party": "🎉", Festival: "🎪", Corporate: "🏢", "Acoustic Set": "🎸", Birthday: "🎂", Other: "🎵" }[type] || "🎵";
}

// ─── Toast ────────────────────────────────────────────────────
function Toast({ message, type }) {
  if (!message) return null;
  const bg = type === "error" ? "#2a1010" : "#0f2218";
  const color = type === "error" ? "#e05c5c" : "#5bb974";
  return (
    <div style={{ position: "fixed", top: 20, left: "50%", transform: "translateX(-50%)", background: bg, color, border: `1px solid ${color}`, borderRadius: 12, padding: "10px 20px", fontSize: 13, fontWeight: 600, zIndex: 999, whiteSpace: "nowrap", boxShadow: "0 4px 20px rgba(0,0,0,0.4)" }}>
      {message}
    </div>
  );
}
...
export default function App() {
  ...
  const fetchStats = useCallback(async () => {
    try {
      const res = await api.get("/gigs/stats");
      if (res.success) setStats(res.data);
    } catch { }
  }, []);

  const refresh = useCallback(async () => {
    await Promise.all([fetchGigs(), fetchStats()]);
  ...
  async function saveGig(form) {
    try {
      const res = form._id
        ? await api.put(`/gigs/${form._id}`, form)
  ...
  async function deleteGig(id) {
    try {
      const res = await api.del(`/gigs/${id}`);
      if (!res.success) { showToast(res.message, "error"); return; }
    ...
    <div style={{ minHeight: "100vh", background: "#1a1a1a", color: "#e8e8e6", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", fontSize: 14 }}>
      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "#111", borderTop: "1px solid #252525", display: "flex", alignItems: "center", padding: "0 8px", zIndex: 50, height: 64 }}>
        ...
        <button className="tap" onClick={() => setScreen({ type: "form", gig: {} })}
          style={{ width: 46, height: 46, borderRadius: 23, background: "#c98a3a", border: "none", color: "#fff", fontSize: 24, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 16px rgba(201,138,58,0.4)", margin: "0 8px" }}>
          +
        </button>
      </div>
    </div>
    ...
    <div style={{ padding: "24px 16px 0" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 24 }}>
        {[
          ...
          { label: "Total Pending", val: fmt(stats.totalPending), color: "#e07b3a" },
...
function GigForm({ gig, onSave, onBack }) {
  const [form, setForm] = useState({
    client: gig?.client || "", date: gig?.date ? new Date(gig.date).toISOString().slice(0, 10) : new Date().toISOString().slice(0, 10),
    type: gig?.type || "Wedding", fee: gig?.fee || "", paid: gig?.paid || 0,
    notes: gig?.notes || "", confirmed: gig?.confirmed ?? false, _id: gig?._id || null,
  });
